  ...
  let  platform = navigator.userAgent;
    ...
    getDevice() {
        ...
        class PXEngine_1_0{
            ...
            api_init_flag;
            PXENGINE_START = this.api_init_flag;
            spriteFiles;
            backgroundFiles;
            GameWindowX;
            GameWindowY;
        ...
        str = {
            message:{
                ...
                video_handle_isValid: "Видео Элемент валиден",
                audio_handle_isValid: "Аудио Элемент валиден"
            }
        };
        status(st){
        if(st){
       ...
       CheckValidVideoTag(idVideoTag){
        if(!idVideoTag instanceof HTMLVideoElement){
            alert(this.str.message.error_video_handle);
        }
        if(idVideoTag instanceof HTMLVideoElement){
            alert(this.str.message.video_handle_isValid);
        }
       }
       CheckValidAudioTag(idAudioTag){
        if(!idAudioTag instanceof HTMLAudioElement){
            alert(this.str.message.error_audio_handle);
        }
        if(idAudioTag instanceof HTMLAudioElement){
            alert(this.str.message.audio_handle_isValid);
        }
       }
       ...
       logL("PXENGINE "+this.PXENGINE_VERSION + " status: OK"+"\ngithub/hcpp20334\nmodule: pxengine_1.0.js")
       }
       reFillMain(){ 
        ...
        class PXEngineDebug{
            ...
            getInfo(){
                this.getStr("<br> device_info: name:"+this.device_info.name,0);
                this.getStr("<br>display_size: ctx"+this.display_size.ctx+" cty:"+this.display_size.cty,0);
                this.getStr("<br>window_size: ctx"+this.window_size.ctx + " cty:"+this.window_size.cty,0);
                this.getStr("<br>_data: state" + this._data.state ? "true" : "false",0);
                this.getStr("<br>_data: version "+ this._data.version,0);
            }
            };
